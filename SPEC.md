...
- **Storage**
  - `zel compress scan [keys...]` — compress candidate PDFs into a run directory and report what each would cost; reads the library, writes only into the run. `--preset` (Ghostscript quality), `--min-bytes`, `--min-saving`, `--text-tolerance`, `--limit`, `--json`. A candidate is accepted only if the output has the same page count, a text layer within tolerance of the original, and a saving above the floor — rejected output is deleted rather than left staged.
  - `zel compress swap <run>` — install a run's accepted files, moving each original into the run's `originals/` first; `--dry-run` first-class, `--yes` gate, all-or-nothing. Refuses when any storage file's md5 no longer matches what the scan recorded, because the staged copy was then built from bytes that no longer exist. Already-swapped entries are skipped, so re-running is safe.
  - `zel compress restore <run>` — the inverse: originals return to storage, compressed files return to staging, so a restored run can be swapped again without re-scanning.
- **Utilities**
