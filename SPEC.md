...
- Colours: Zotero pins up to 9 coloured tags atop its tag-selector pane and gives them number-key shortcuts — prime UI real estate for workflow tags. A registry entry may declare a colour and position. A declared colour must be a well-formed `#RRGGBB` hex string — the validator rejects typos (`blue`, `#GGGGGG`, a 3-digit shorthand) but not hues: which palette a colour comes from is the user's call, since the registry has to be able to name the colours Zotero itself assigns. Okabe-Ito (ref: jfly.uni-koeln.de/color) is the recommended default and what `taxonomy.example.yaml` ships, a recommendation carried in that file's comments rather than in code. The registry is authoritative for the entire `tagColors` array — the setting is one value, so writing it replaces every assignment, including colours hand-set outside the registry (the plan's old value shows exactly what goes). When registry colours differ from the library setting, `zel validate` adds a library-settings entry (old and new `tagColors` value) to the plan alongside the per-item operations; `zel apply` writes it via the Web API settings endpoint (version-pinned like any write), the change log tracks it, and undo restores the previous value. The validator enforces the 9-tag cap; colours are reserved for workflow families (`status:` and kin), never `topic:`.
...
Default colour assignments, shipped in `taxonomy.example.yaml` (Okabe-Ito by recommendation, not by rule; the copied registry is the user's file, so changing them is editing one line). Positions double as Zotero's number-key shortcuts, ordered by expected frequency of use. Yellow (`#F0E442`) is deliberately unused — coloured tags render as coloured text in the selector, where yellow is illegible:
...
tags:
  - tag: status:read
    ...
    colour: "#009E73"    # optional; any #RRGGBB hex; position = order within coloured tags
```
...
- **Palette enforcement** — the validator once accepted only the eight Okabe-Ito hues, which meant a registry could not name the colours Zotero itself assigns; it blocked every write for a user whose live `tagColors` were correct. Well-formedness is the checkable property, palette is taste, and shipping alternate palettes (Paul Tol and kin) was configurability no session needed either. *Revive if:* a hue check ever has a reason to fail a colour the user deliberately chose — hard to imagine.
